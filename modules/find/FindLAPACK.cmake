...
if(BLAS_FOUND)
  ...
  if (BLAS_LIBRARIES AND NOT LAPACK_STANDALONE)
    ...
    if (LAPACK_LIBRARIES)
      ...
      set(LAPACK_INCLUDE_DIRS "${BLAS_INCLUDE_DIRS}")
      set(LAPACK_CFLAGS_OTHER "${BLAS_CFLAGS_OTHER}")
      set(LAPACK_LIBRARY_DIRS "${BLAS_LIBRARY_DIRS}")
      set(LAPACK_LDFLAGS_OTHER "${BLAS_LDFLAGS_OTHER}")
      if(NOT LAPACK_FIND_QUIETLY)
        if(LAPACK_LIBRARIES)
  ...
  if( (NOT LAPACK_FOUND_WITH_PKGCONFIG) OR LAPACK_GIVEN_BY_USER )
    ...
    if (LAP_VENDOR STREQUAL "Open" OR LAP_VENDOR STREQUAL "All")
      if(NOT LAPACK_LIBRARIES)
        ...
        mark_as_advanced(LAPACK_lapacke.h_DIRS)

        # If found, add path to cmake variable
        # ------------------------------------
        if (LAPACK_lapacke.h_DIRS)
          set(LAPACK_INCLUDE_DIRS "${LAPACK_lapacke.h_DIRS}")
        else ()
          set(LAPACK_INCLUDE_DIRS "LAPACK_INCLUDE_DIRS-NOTFOUND")
          if(NOT LAPACK_FIND_QUIETLY)
            message(STATUS "Looking for LAPACK -- lapacke.h not found")
          endif()
        endif()
