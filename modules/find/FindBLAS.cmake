...
if( PKG_CONFIG_EXECUTABLE AND NOT BLAS_GIVEN_BY_USER AND BLA_PREFER_PKGCONFIG)
  ...
  if ( (BLA_VENDOR MATCHES "Intel*") AND
      (NOT BLA_F95) AND NOT ((_LANGUAGES_ MATCHES Fortran) AND
        (CMAKE_Fortran_COMPILER_ID STREQUAL "GNU")) AND NOT (CMAKE_C_COMPILER_ID
        STREQUAL "GNU"))
  ...
  elseif(BLA_VENDOR STREQUAL "All")
    if ((NOT BLAS_FOUND) AND (NOT BLA_F95) AND NOT ((_LANGUAGES_ MATCHES
            Fortran) AND (CMAKE_Fortran_COMPILER_ID STREQUAL "GNU")) AND NOT
...
if( (NOT BLAS_FOUND_WITH_PKGCONFIG) OR BLAS_GIVEN_BY_USER )
  ...
  if (BLA_VENDOR STREQUAL "Open" OR BLA_VENDOR STREQUAL "All")
    ...
    if(NOT BLAS_LIBRARIES)
      ...
      # ------------------------------------------
      unset(_inc_env)
      set(ENV_OPENBLAS "$ENV{OPENBLAS_DIR}")
      set(ENV_BLAS_DIR "$ENV{BLAS_DIR}")
      set(ENV_BLAS_INCDIR "$ENV{BLAS_INCDIR}")
      if(ENV_BLAS_INCDIR)
        list(APPEND _inc_env "${ENV_BLAS_INCDIR}")
      elseif(ENV_BLAS_DIR)
        ...
        if (ENV_OPENBLAS)
          list(APPEND _inc_env "${ENV_OPENBLAS}/include")
        endif()
        # system variables
        if(WIN32)
          string(REPLACE ":" ";" _path_env "$ENV{INCLUDE}")
      ...
      # Try to find the openblas header in the given paths
      # --------------------------------------------------
      # call cmake macro to find the header path
      if(BLAS_INCDIR)
      ...
      mark_as_advanced(BLAS_cblas.h_DIRS)

      # If found, add path to cmake variable
      # ------------------------------------
      if (BLAS_cblas.h_DIRS)
        set(BLAS_INCLUDE_DIRS "${BLAS_cblas.h_DIRS}")
      else ()
        set(BLAS_INCLUDE_DIRS "BLAS_INCLUDE_DIRS-NOTFOUND")
        if(NOT BLAS_FIND_QUIETLY)
          message(STATUS "Looking for BLAS -- cblas.h not found")
        endif()
      endif()
...
if (BLAS_FOUND)
  # set BLAS_LIBRARY_DIRS and BLAS_DIR_FOUND
  ...
  if (NOT BLAS_FIND_QUIETLY)
    message(STATUS "Check Fortran mangling for BLAS symbols")
  endif()
  set(CMAKE_REQUIRED_LIBRARIES "${BLAS_LIBRARIES}")
  check_function_exists(sgemm BLAS_NOCHANGE)
  check_function_exists(sgemm_ BLAS_ADD_)
  check_function_exists(SGEMM BLAS_UPCASE)
endif()
